...
if (isset($_POST['password'])) {
    if ($_POST['password'] === $settings['admin_password']) $_SESSION['authenticated'] = true;
...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] === 'delete_history_item') {
        ...
        $newHistory = array_values(array_filter($history, function($item) use ($id) {
            return ($item['id'] ?? '') !== $id;
        }));
        Storage::write('history.json', $newHistory);
