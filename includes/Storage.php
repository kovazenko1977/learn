...
class Storage {
    ...
    private $whitelist = [
        'users' => ['id', 'username', 'password', 'full_name', 'role', 'department', 'email', 'telegram_id', 'last_seen', 'banned', 'created_at'],
        'tasks' => ['id', 'title', 'description', 'category', 'priority', 'status', 'creator_id', 'executor_id', 'deadline', 'completed_at', 'attachments', 'custom_fields', 'created_at', 'updated_at'],
        'comments' => ['id', 'task_id', 'user_id', 'content', 'attachments', 'created_at'],
        'recovery' => ['id', 'user_id', 'expires'],
        'chat' => ['id', 'sender_id', 'recipient_id', 'message', 'attachment_url', 'attachment_name', 'created_at']
    ];
    ...
    public function getAll($collection) {
        if (!isset($this->whitelist[$collection])) {
            return [];
        }
        if ($this->mode === 'mysql') {
            try {
    ...
    public function save($collection, $data) {
        if (!isset($this->whitelist[$collection])) {
            throw new Exception("Invalid collection: " . $collection);
        }
        // Strict whitelist filtering of data keys to block SQLi and unvalidated column injections
        $data = array_filter($data, fn($key) => in_array($key, $this->whitelist[$collection]), ARRAY_FILTER_USE_KEY);
