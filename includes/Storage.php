...
class Storage {
    ...
    private $allowedFields = [
        ...
        'tasks' => ['id', 'title', 'description', 'category', 'priority', 'status', 'creator_id', 'executor_id', 'deadline', 'custom_fields', 'created_at', 'updated_at', 'completed_at', 'attachment'],
        'comments' => ['id', 'task_id', 'user_id', 'text', 'created_at'],
        'history' => ['id', 'task_id', 'user_id', 'old_status', 'new_status', 'created_at'],
        'recoveries' => ['token', 'user_id', 'expires']
    ];
    ...
    private function validateCollection($collection) {
        if (!array_key_exists($collection, $this->allowedFields)) {
            throw new InvalidArgumentException("Invalid collection: " . $collection);
        }
    }

    private function filterData($collection, $data) {
        $this->validateCollection($collection);
        $fields = $this->allowedFields[$collection];
        $filtered = [];
        foreach ($data as $k => $v) {
    ...
    public function get($collection, $where = []) {
        ...
        if ($this->mode === 'mysql' && $this->pdo) {
            ...
            if (!empty($where)) {
                $clauses = [];
                $filteredWhere = $this->filterData($collection, $where);
    ...
    public function save($collection, $data) {
        $this->validateCollection($collection);
        $filtered = $this->filterData($collection, $data);
        ...
        if ($this->mode === 'mysql' && $this->pdo) {
            ...
            if (isset($toSave['id'])) {
                ...
                $keys = implode(', ', array_map(fn($k) => "`$k`", array_keys($toSave)));
