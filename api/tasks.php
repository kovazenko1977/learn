...
if ($method === 'GET') {
    if ($id) {
        ...
        if ($currentUser['role'] === 'Administrator') {
            ...
            $hasAccess = (($task['department'] ?? '') === ($currentUser['department'] ?? ''));
        } elseif ($currentUser['role'] === 'Executor') {
            $hasAccess = (($task['executor_id'] ?? '') === $currentUser['id']);
        } elseif ($currentUser['role'] === 'Responsible Employee') {
            $hasAccess = (($task['creator_id'] ?? '') === $currentUser['id']);
        }
    ...
    if ($id && isset($data['content'])) { // Add comment
        ...
        if ($id) {
            ...
            if ($currentUser['role'] === 'Executor' && isset($data['status'])) {
                // Executor can only change status on tasks assigned to them
                if (($existing['executor_id'] ?? '') !== $currentUser['id']) {
                    http_response_code(403);
                    echo json_encode(['success' => false, 'message' => 'Forbidden']);
                    exit;
                }
                ...
                if ($currentUser['role'] === 'Department Head' && ($existing['department'] ?? '') !== ($currentUser['department'] ?? '')) {
                    http_response_code(403);
                    echo json_encode(['success' => false, 'message' => 'Forbidden']);
                    exit;
                }
