...
import { Plus, Trash2, ArrowUp, ArrowDown, Save, ToggleLeft, ToggleRight, SquarePlay, Move } from 'lucide-react';
...
export const FormBuilder: React.FC = () => {
  ...
  // Reorder sort order using up/down buttons
  ...
  // HTML5 Drag-and-Drop Handlers for drag reordering fields
  ...
  const handleDrop = async (e: React.DragEvent, targetField: FormField) => {
    e.preventDefault();
    ...
    // Swap sort orders
    const originalSort = draggedField.sort_order;
    const targetSort = targetField.sort_order;
    ...
    try {
      ...
      await fetch('/api/admin/form-fields', {
        ...
        body: JSON.stringify({ ...draggedField, sort_order: targetSort })
      });
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200/50 dark:border-slate-800/50 shadow-sm lg:col-span-2 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-3 border-b border-slate-100 dark:border-slate-800">
          <div>
            ...
            <span className="text-[10px] text-slate-400 font-medium">Перетащите поля мышкой или используйте стрелки для изменения порядка</span>
          </div>
          <select
            value={selectedCategory}
            onChange={(e) => setSelectedCategory(e.target.value)}
        ...
        <div className="space-y-2.5">
          {filteredFields.map((field, index) => (
            <div
              ...
              className="p-4 bg-slate-50 dark:bg-slate-950/30 border border-slate-200/50 dark:border-slate-800/30 rounded-xl flex items-center justify-between text-xs cursor-grab active:cursor-grabbing hover:border-primary-500/50 hover:bg-slate-100/50 dark:hover:bg-slate-800/20 transition duration-150"
            >
              <div className="flex items-center gap-3">
                <Move className="w-4 h-4 text-slate-400 cursor-grab active:cursor-grabbing" />
                <div>
