...
<div class="m-card" style="padding: 12px; margin-bottom: 12px;">
    <form method="get" style="display: flex; gap: 8px; align-items: center;">
        ...
        <button type="submit" class="btn-m btn-m-primary" style="padding: 6px 16px; width: auto; font-size: 0.8rem;">Показат../button>
   ../form>
</div>
...
<div style="margin-top: 15px; display: flex; gap: 15px; font-size: 0.7rem; color: #64748b; padding: 0 5px;">
    <div style="display:flex; align-items:center; gap:4px;"><div style="width:10px; height:10px; background:#fff3cd; border-radius:2px;"../div> Резер../div>
    <div style="display:flex; align-items:center; gap:4px;"><div style="width:10px; height:10px; background:#fee2e2; border-radius:2px;"../div> Заня../div>
</div>
    ...
    <div style="background:#fff; width:100%; border-radius: 20px 20px 0 0; padding: 20px 20px calc(20px + var(--safe-area-bottom)); box-sizing: border-box; max-height: 85vh; overflow-y: auto;">
       ...
       ../div>
        ...
        <div style="margin-top: 25px; display: flex; flex-direction: column; gap: 10px;">
            ...
            <button class="btn-m" onclick="closeDetails()" style="background:#f1f5f9; color:#1e293b; margin-top: 5px;">Закрыт../button>
       ../div>
   ../div>
</div>
...
function showBookingDetails(id, roomNum) {
    ...
    let html = `
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px;">
            <div>
                <div style="font-size: 0.75rem; color: #64748b; margin-bottom: 4px;">Гост../div>
                <div style="font-weight: 800; font-size: 1.2rem; color: #1e293b;">${b.client_name || ../A'../div>
                <div style="color: var(--mobile-primary); font-weight: 600; font-size: 0.95rem; margin-top: 2px;">${b.phone || ''../div>
           ../div>
            <div style="text-align: right;">
                <div style="font-size: 0.75rem; color: #64748b; margin-bottom: 4px;">Номе../div>
       ...
       ../div>
        ...
        <div class="m-stats-grid" style="margin-bottom: 20px;">
            <div class="m-stat-item" style="text-align: left; padding: 10px;">
                <div class="m-stat-label">Заез../div>
                <div style="font-weight: 700; color: #1e293b;">${b.check_in../div>
           ../div>
            <div class="m-stat-item" style="text-align: left; padding: 10px;">
                <div class="m-stat-label">Выез../div>
       ...
       ../div>
        ...
        <div style="margin-bottom: 15px; padding: 12px; background: #f8fafc; border-radius: 10px;">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <span style="font-size: 0.85rem; color: #64748b;">Стату../span>
                <span class="badge-m ${statusClass}">${statusLabel../span>
           ../div>
            <div style="margin-top: 10px; display: flex; justify-content: space-between; align-items: center;">
                <span style="font-size: 0.85rem; color: #64748b;">Паке../span>
       ...
       ../div>
        ...
        <div style="margin-bottom: 15px; padding: 12px; background: #fffbeb; border: 1px solid #fef3c7; border-radius: 10px;">
            <div style="font-size: 0.7rem; color: #b45309; text-transform: uppercase; letter-spacing: 0.05em; font-weight: 700; margin-bottom: 5px;">Заметк../div>
  ...
  ..// Actions
    ...
    if (b.status === 'reserved') {
        ...
        actionsHtml += `<button class="btn-m" style="background: #f97316; color: white;" onclick="updateBookingStatus(\${id}, 'confirmed')">Оформить выез../button>`;
    }

    if (b.status !== 'cancelled' && b.status !== 'confirmed') {
        actionsHtml += `<button class="btn-m" style="background: #fee2e2; color: #991b1b;" onclick="updateBookingStatus(\${id}, 'cancelled')">Отменить бронировани../button>`;
    }
