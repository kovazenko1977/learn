...
class Connection(Base):
    ...
    params: Mapped[str] = mapped_column(Text)  # JSON encoded parameters
...
class Relay(Base):
    ...
    number: Mapped[int] = mapped_column(Integer)
    name: Mapped[str] = mapped_column(String(100))
    program: Mapped[int] = mapped_column(Integer, default=0) # Logic program ID
...
class AccessLevel(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    name: Mapped[str] = mapped_column(String(100))
    # Define bitmask or associations for zone rights

class Key(Base):
    __tablename__ = "keys"
    id: Mapped[int] = mapped_column(primary_key=True)
    code: Mapped[str] = mapped_column(String(50), unique=True)
    type: Mapped[str] = mapped_column(String(20)) # PIN, RFID, iButton
...
class Scenario(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    name: Mapped[str] = mapped_column(String(100))
    definition: Mapped[str] = mapped_column(Text) # JSON structure of the scenario logic
