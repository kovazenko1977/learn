...
namespace App\Modules\Transfer_Pro;
use App\Module\BaseModule;
use App\Core\Router;
class Module extends BaseModule {
    public function boot(): void {
        $router = $this->container->get(Router::class);
        $router->addRoute('GET', '/' . strtolower('Transfer_Pro'), [$this, 'index']);
    }
    public function index($request, $response): string {
        $renderer = $this->container->get(\App\View\Renderer::class);
        $content = "<div class='mb-8'>
            <h2 class='text-4xl font-extrabold text-gray-900'>Transfer_Pro</h2>
            <p class='text-gray-500 mt-2'>Трансферы и логистика. Модуль готов к работе.</p>
        </div>
        ...
        return $renderer->render('layout', [
            'title' => 'Transfer_Pro - VSPRINT 2.0',
            'content' => $content,
            'user' => ['username' => 'Admin']
        ]);
    }
