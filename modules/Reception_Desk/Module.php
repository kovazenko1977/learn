...
namespace App\Modules\Reception_Desk;
use App\Module\BaseModule;
use App\Core\Router;
class Module extends BaseModule {
    public function boot(): void {
        $router = $this->container->get(Router::class);
        $router->addRoute('GET', '/' . strtolower('Reception_Desk'), [$this, 'index']);
    }
    public function index($request, $response): string {
        $renderer = $this->container->get(\App\View\Renderer::class);
        $content = "<div class='mb-8'>
            <h2 class='text-4xl font-extrabold text-gray-900'>Reception_Desk</h2>
            <p class='text-gray-500 mt-2'>Ресепшн и прием. Модуль готов к работе.</p>
        </div>
        ...
        return $renderer->render('layout', [
            'title' => 'Reception_Desk - VSPRINT 2.0',
            'content' => $content,
            'user' => ['username' => 'Admin']
        ]);
    }
