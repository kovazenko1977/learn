...
class Module extends BaseModule {
    public function boot(): void {
        ...
        $router->addRoute('GET', '/' . strtolower('HelpDesk'), [$this, 'index']);
    }
    public function index($request, $response): string {
        $renderer = $this->container->get(\App\View\Renderer::class);
        $content = "<div class='mb-8'>
            <h2 class='text-4xl font-extrabold text-gray-900'>HelpDesk</h2>
            <p class='text-gray-500 mt-2'>Техническая поддержка. Модуль готов к работе.</p>
        </div>
