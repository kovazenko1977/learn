...
const translations = {
    ru: {
        ...
        token_from_email: "Токен из письма",
        new_password: "Новый пароль",
        total_tasks: "Всего заявок",
        in_work_label: "В работе",
        done_label: "Выполнено",
        avg_time: "Среднее время",
        task_statuses: "Статусы заявок",
        executor_load: "Нагрузка по исполнителям",
        no_executor_data: "Нет данных по исполнителям",
        tasks_count: "задач",
        ...
        storage_mode: "Режим хранения",
        storage_json: "JSON (Локальные файлы)",
        storage_mysql: "MySQL (База данных)",
        task_categories: "Категории заявок",
        new_category_placeholder: "Новая категория",
        new_field_placeholder: "Новое поле",
        form_constructor: "Конструктор полей формы",
        field_label_placeholder: "Название поля",
        type_text: "Текст",
        type_tel: "Телефон",
        type_number: "Число",
        type_date: "Дата",
        ...
        notif_created: "Новая заявка",
        notif_assignment: "Назначение",
        notif_status_change: "Смена статуса",
        notif_frequency: "Частота уведомлений",
        notif_immediate: "Мгновенно",
        notif_hourly: "Раз в час",
        notif_daily: "Раз в день",
        sla_label: "SLA (Сроки в часах)",
        save_all_settings: "Сохранить все настройки",
        full_name: "ФИО",
        telegram_id: "Telegram ID",
        new_password_help: "Новый пароль (оставьте пустым, чтобы не менять)",
        role_admin: "Администратор",
        role_responsible: "Ответственный сотрудник",
        role_head: "Начальник отдела",
        role_executor: "Исполнитель (мастер, техник)",
    ...
    en: {
        ...
        token_from_email: "Token from email",
        new_password: "New Password",
        total_tasks: "Total Tasks",
        in_work_label: "In Progress",
        done_label: "Done",
        avg_time: "Avg Time",
        task_statuses: "Task Statuses",
        executor_load: "Executor Load",
        no_executor_data: "No executor data",
        tasks_count: "tasks",
        ...
        storage_json: "JSON (Local Files)",
        storage_mysql: "MySQL (Database)",
        task_categories: "Task Categories",
        new_category_placeholder: "New Category",
        new_field_placeholder: "New Field",
        form_constructor: "Form Field Constructor",
        ...
        notif_created: "New Task",
        notif_assignment: "Assignment",
        notif_status_change: "Status Change",
        notif_frequency: "Notification Frequency",
        notif_immediate: "Immediate",
        notif_hourly: "Hourly",
        notif_daily: "Daily",
        sla_label: "SLA (Hours)",
        ...
        role_responsible: "Responsible Employee",
    ...
    template: `
        <div class="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-xl w-full max-w-md">
            ...
            <form v-if="mode === 'login'" @submit.prevent="submit" class="space-y-4">
                ...
                <button type="button" @click="mode = 'recover'" class="w-full text-sm text-primary hover:underline">{{ t('forgot_password') }}</button>
            </form>

            <form v-if="mode === 'recover'" @submit.prevent="submit" class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('your_email') }}</label>
                ...
                <button type="submit" class="w-full bg-primary text-white font-bold py-2 px-4 rounded-md hover:bg-blue-600 transition-colors">{{ t('recover_btn') }}</button>
                <button type="button" @click="mode = 'login'" class="w-full text-sm text-gray-500 hover:underline">{{ t('back_to_login') }}</button>
            </form>

            <form v-if="mode === 'reset'" @submit.prevent="submit" class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('token_from_email') }}</label>
                ...
                <button type="submit" class="w-full bg-primary text-white font-bold py-2 px-4 rounded-md hover:bg-blue-600 transition-colors">{{ t('save') }}</button>
            </form>
        </div>
    `
});
        ...
        <div>
            ...
            <div v-if="stats" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border-l-4 border-blue-500">
                    <div class="text-gray-500 dark:text-gray-400 text-sm font-medium">{{ t('total_tasks') }}</div>
                ...
                <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border-l-4 border-yellow-500">
                    <div class="text-gray-500 dark:text-gray-400 text-sm font-medium">{{ t('in_work_label') }}</div>
                    <div class="text-3xl font-bold dark:text-white">{{ (stats.by_status.in_work || 0) + (stats.by_status.assigned || 0) }}</div>
                </div>
                <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border-l-4 border-green-500">
                    <div class="text-gray-500 dark:text-gray-400 text-sm font-medium">{{ t('done_label') }}</div>
                    <div class="text-3xl font-bold dark:text-white">{{ stats.by_status.done || 0 }}</div>
                </div>
                <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border-l-4 border-purple-500">
                    <div class="text-gray-500 dark:text-gray-400 text-sm font-medium">{{ t('avg_time') }}</div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm">
                    <h3 class="text-lg font-semibold mb-4 dark:text-white">{{ t('task_statuses') }}</h3>
                    <div class="space-y-4">
                        <div v-for="(val, key) in stats?.by_status" :key="key" class="flex items-center justify-between">
                            ...
                            <div class="flex-1 mx-4 h-2 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                                <div class="h-full bg-primary" :style="{ width: (stats.total > 0 ? (val/stats.total*100) : 0) + '%' }"></div>
                            </div>
                ...
                <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm">
                    <h3 class="text-lg font-semibold mb-4 dark:text-white">{{ t('executor_load') }}</h3>
                    <div v-if="stats?.by_executor && Object.keys(stats.by_executor).length > 0" class="space-y-4">
                        <div v-for="(val, id) in stats.by_executor" :key="id" class="flex items-center justify-between">
                            ...
                            <span class="font-bold dark:text-white">{{ val }} {{ t('tasks_count') }}</span>
                        </div>
                    </div>
                    <div v-else class="text-gray-400 italic">{{ t('no_executor_data') }}</div>
                </div>
            </div>
        </div>
    `
...
app.component('tasks-view', {
    ...
    setup(props, { emit }) {
        ...
        const statusColors = {
            new: 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200 border-blue-500',
            assigned: 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-200 border-yellow-500',
            in_work: 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-800 dark:text-indigo-200 border-indigo-500',
            done: 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200 border-green-500',
            rejected: 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-200 border-red-500'
        };
        ...
        const isOverdue = (task) => {
            if (['done', 'rejected'].includes(task.status)) return false;
            if (!task.deadline) return false;
            return new Date(task.deadline) < new Date();
        };

        return { showForm, searchQuery, filterStatus, showArchive, filteredTasks, statusColors, updateStatus, selectedTask, onDragEnd, print, selectedTasks, bulkAssign, isOverdue };
    },
    template: `
        <div>
            <div v-if="selectedTasks.length > 0" class="fixed bottom-6 left-1/2 -translate-x-1/2 bg-white dark:bg-gray-800 shadow-2xl rounded-full px-6 py-3 flex items-center space-x-6 z-50 border border-primary no-print">
                <span class="text-sm font-bold dark:text-white">{{ t('selected_count') }}: {{ selectedTasks.length }}</span>
                <div class="flex items-center space-x-2">
                    <span class="text-xs text-gray-500">{{ t('assign_label') }}:</span>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 mb-6 flex flex-wrap gap-4 no-print">
                <input v-model="searchQuery" type="text" :placeholder="t('search_placeholder')" class="border border-gray-300 dark:border-gray-600 rounded-md p-2 dark:bg-gray-700 dark:text-white flex-1 min-w-[200px]">
                ...
                <button @click="showArchive = !showArchive" :class="['px-4 py-2 rounded-md transition-colors', showArchive ? 'bg-purple-500 text-white' : 'bg-gray-100 dark:bg-gray-700 dark:text-gray-200']">
                    <i class="fas" :class="showArchive ? 'fa-box-open' : 'fa-archive'"></i> {{ showArchive ? t('current_tasks') : t('archive') }}
                </button>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8 overflow-x-auto">
                <div v-for="status in (showArchive ? ['done', 'rejected'] : ['new', 'assigned', 'in_work'])" :key="status" class="bg-gray-100 dark:bg-gray-900 p-3 rounded-lg min-w-[250px] kanban-column">
                    <h3 class="font-bold mb-4 flex justify-between dark:text-gray-200 text-sm">
                        {{ t('status_' + status) }}
                    ...
                    <draggable :list="tasks.filter(t => t.status === status)"
                        ...
                        <template #item="{element}">
                            <div class="bg-white dark:bg-gray-800 p-4 rounded-md shadow-sm border-l-4 cursor-pointer hover:shadow-md transition-shadow relative"
                                 :class="[statusColors[element.status], isOverdue(element) ? 'border-red-500 bg-red-50 dark:bg-red-950/20' : '']"
                                ...
                                <div class="font-semibold text-gray-800 dark:text-white truncate pr-6 text-sm">{{ element.title }}</div>
                                <div class="text-xs text-gray-500 dark:text-gray-400 mt-1 flex justify-between items-center">
                                    <span>{{ element.category }} | {{ element.priority }}</span>
                                    <span v-if="isOverdue(element)" class="bg-red-600 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">{{ t('overdue_label') }}</span>
                                </div>
                                <div class="mt-3 flex justify-between items-center text-[10px] text-gray-400 dark:text-gray-500">
                                ...
                                <div v-if="element.deadline" class="mt-1 text-[9px] text-red-500 dark:text-red-400 font-medium">
                                    {{ t('sla_deadline') }}: {{ element.deadline }}
                                </div>
                            </div>
                        </template>
                    </draggable>
            ...
            <div class="print-only">
                <h1 class="text-3xl font-bold mb-8">{{ t('report_title') }}</h1>
                <table class="w-full border-collapse border border-gray-300">
                    <thead>
                        <tr class="bg-gray-100">
                            <th class="border p-2">{{ t('id') }}</th>
                            <th class="border p-2">{{ t('title') }}</th>
                            <th class="border p-2">{{ t('status') }}</th>
                            <th class="border p-2">{{ t('priority') }}</th>
                        </tr>
                    </thead>
                    <tbody>
        ...
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div class="bg-white dark:bg-gray-800 p-6 rounded-xl w-full max-w-lg">
                ...
                <form @submit.prevent="submit" class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium dark:text-gray-300">{{ t('title') }}</label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium dark:text-gray-300">{{ t('priority') }}</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium dark:text-gray-300">{{ t('description') }}</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium dark:text-gray-300">{{ t('attach_files') }}</label>
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-auto">
                <div class="flex justify-between items-start mb-6">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ task.category }} | {{ task.priority }} | {{ task.status }}</p>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="md:col-span-2 space-y-4">
                        <div class="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
                            <h4 class="font-bold mb-2 dark:text-gray-200">{{ t('description') }}</h4>
                        ...
                        <div v-if="task.attachments && task.attachments.length" class="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
                             <h4 class="font-bold mb-2 dark:text-gray-200">{{ t('attachments') }}</h4>
                        ...
                        <div v-if="task.custom_fields && Object.keys(task.custom_fields).length" class="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
                             <h4 class="font-bold mb-2 dark:text-gray-200">{{ t('custom_fields_label') }}</h4>
                        ...
                        <div>
                            <h4 class="font-bold mb-4 dark:text-gray-200">{{ t('comments') }}</h4>
                            ...
                            <div class="flex space-x-2">
                                <input v-model="comment" type="text" :placeholder="t('comment_placeholder')" class="flex-1 border border-gray-300 dark:border-gray-600 rounded-md p-2 dark:bg-gray-700 dark:text-white">
                    ...
                    <div class="space-y-6">
                        <div>
                            <h4 class="font-bold mb-2 dark:text-gray-200">{{ t('management') }}</h4>
                            <div class="space-y-2">
                                <button v-if="task.status !== 'done'" @click="changeStatus('done')" class="w-full bg-green-500 text-white py-2 rounded-md hover:bg-green-600">{{ t('complete_btn') }}</button>
                                <button v-if="task.status === 'new'" @click="changeStatus('in_work')" class="w-full bg-indigo-500 text-white py-2 rounded-md hover:bg-indigo-600">{{ t('start_work_btn') }}</button>
                                <button v-if="task.status !== 'rejected'" @click="changeStatus('rejected')" class="w-full bg-red-100 text-red-600 py-2 rounded-md hover:bg-red-200">{{ t('reject_btn') }}</button>
                            </div>
                        </div>

                        <div v-if="isAdminOrHead">
                            <h4 class="font-bold mb-2 dark:text-gray-200">{{ t('assign_executor') }}</h4>
                        ...
                        <div class="text-[10px] text-gray-400 space-y-1">
                            <div>{{ t('created_at_label') }}: {{ task.created_at }}</div>
                            <div v-if="task.deadline" class="text-red-400 font-bold">{{ t('deadline_label') }}: {{ task.deadline }}</div>
                        </div>
                    </div>
                </div>
            </div>
...
app.component('users-view', {
    ...
    setup(props, { emit }) {
        ...
        const remove = async (id) => {
            if (confirm(props.t('delete_confirm'))) {
        ...
        const localizedRole = (role) => {
            const rolesMap = {
                'Administrator': props.t('role_admin'),
                'Responsible Employee': props.t('role_responsible'),
                'Department Head': props.t('role_head'),
                'Executor': props.t('role_executor')
            };
            return rolesMap[role] || role;
        };

        return { showForm, editingUser, roles, form, edit, save, remove, localizedRole };
    },
    template: `
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
            <div class="p-6 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
                <h2 class="text-xl font-bold dark:text-white">{{ t('users') }}</h2>
                <button @click="editingUser = null; showForm = true" class="bg-primary text-white px-4 py-2 rounded-md hover:bg-blue-600">{{ t('add') }}</button>
            </div>
            <table class="w-full text-left">
                <thead class="bg-gray-50 dark:bg-gray-900 text-gray-500 uppercase text-xs">
                    <tr>
                        <th class="px-6 py-3">{{ t('user_label') }}</th>
                        <th class="px-6 py-3">{{ t('role') }}</th>
                        <th class="px-6 py-3">{{ t('department') }}</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    <tr v-for="u in users" :key="u.id" class="hover:bg-gray-50 dark:hover:bg-gray-900 dark:text-gray-200">
                        ...
                        <td class="px-6 py-4 text-sm">{{ localizedRole(u.role) }}</td>
            ...
            <div v-if="showForm" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div class="bg-white dark:bg-gray-800 p-6 rounded-xl w-full max-w-md">
                    <h3 class="text-xl font-bold mb-4 dark:text-white">{{ editingUser ? 'Edit' : t('add') }} {{ t('user_label') }}</h3>
                    <form @submit.prevent="save" class="space-y-4">
                        <input v-model="form.username" :placeholder="t('username')" class="w-full border p-2 rounded dark:bg-gray-700 dark:text-white" required>
                        <input v-model="form.password" type="password" :placeholder="t('password')" class="w-full border p-2 rounded dark:bg-gray-700 dark:text-white" :required="!editingUser">
                        <input v-model="form.full_name" :placeholder="t('full_name')" class="w-full border p-2 rounded dark:bg-gray-700 dark:text-white" required>
                        <select v-model="form.role" class="w-full border p-2 rounded dark:bg-gray-700 dark:text-white">
                            <option v-for="r in roles" :value="r">{{ localizedRole(r) }}</option>
                        </select>
                        <input v-model="form.department" :placeholder="t('department')" class="w-full border p-2 rounded dark:bg-gray-700 dark:text-white">
                        ...
                        <div class="flex justify-end space-x-2">
                            <button type="button" @click="showForm = false" class="text-gray-500">{{ t('cancel') }}</button>
                            <button type="submit" class="bg-primary text-white px-4 py-2 rounded">{{ t('save') }}</button>
                        </div>
                    </form>
                </div>
            </div>
...
app.component('settings-view', {
    ...
    setup(props, { emit }) {
        ...
        const addCategory = () => localSettings.categories.push(props.t('new_category_placeholder'));
        const addField = () => localSettings.form_fields.push({ id: 'f' + Date.now(), label: props.t('new_field_placeholder'), type: 'text', required: false });
    ...
    template: `
        <div class="max-w-4xl mx-auto space-y-6">
            <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm">
                ...
                <div class="space-y-8">
                    <div>
                        <h3 class="font-bold mb-4 dark:text-gray-300">{{ t('storage_mode') }}</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <select v-model="localSettings.storage_mode" class="border p-2 rounded dark:bg-gray-700 dark:text-white w-full">
                                    <option value="json">{{ t('storage_json') }}</option>
                                    <option value="mysql">{{ t('storage_mysql') }}</option>
                                </select>
                            </div>
                    ...
                    <div>
                        <h3 class="font-bold mb-4 dark:text-gray-300">{{ t('task_categories') }}</h3>
                    ...
                    <div>
                        <h3 class="font-bold mb-4 dark:text-gray-300">{{ t('form_constructor') }}</h3>
                        <div class="space-y-2">
                            <draggable v-model="localSettings.form_fields" item-key="id" handle=".fa-grip-lines">
                                <template #item="{element, index}">
                                    <div class="flex items-center space-x-2 bg-gray-50 dark:bg-gray-900 p-2 rounded mb-2">
                                        ...
                                        <input v-model="element.label" class="flex-1 border-none bg-transparent dark:text-white" :placeholder="t('field_label_placeholder')">
                                        ...
                                        <label class="flex items-center text-xs dark:text-gray-400">
                                            <input type="checkbox" v-model="element.required" class="mr-1"> {{ t('required_label') }}
                                        </label>
                                        <button @click="localSettings.form_fields.splice(index,1)" class="text-red-500"><i class="fas fa-trash"></i></button>
                                    </div>
                                </template>
                            </draggable>
                            <button @click="addField" class="text-primary text-sm font-bold">+ {{ t('add') }}</button>
                        </div>
                    </div>

                    <div>
                        <h3 class="font-bold mb-4 dark:text-gray-300">{{ t('notifications') }}</h3>
                        <div class="space-y-4 border p-4 rounded bg-gray-50 dark:bg-gray-900">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2 border-t dark:border-gray-700">
                                <label class="flex items-center text-sm dark:text-gray-300">
                                    <input type="checkbox" v-model="localSettings.notifications.triggers.created" class="mr-2"> {{ t('notif_created') }}
                                </label>
                                <label class="flex items-center text-sm dark:text-gray-300">
                                    <input type="checkbox" v-model="localSettings.notifications.triggers.assignment" class="mr-2"> {{ t('notif_assignment') }}
                                </label>
                                <label class="flex items-center text-sm dark:text-gray-300">
                                    <input type="checkbox" v-model="localSettings.notifications.triggers.status_change" class="mr-2"> {{ t('notif_status_change') }}
                                </label>
                            </div>
                            <div class="pt-4 border-t dark:border-gray-700">
                                <label class="block text-sm font-medium dark:text-gray-300 mb-2">{{ t('notif_frequency') }}</label>
                                <select v-model="localSettings.notifications.frequency" class="w-full border p-2 rounded dark:bg-gray-700 dark:text-white text-sm">
                                    <option value="immediate">{{ t('notif_immediate') }}</option>
                                    <option value="hourly">{{ t('notif_hourly') }}</option>
                    ...
                    <div>
                        <h3 class="font-bold mb-4 dark:text-gray-300">{{ t('sla_label') }}</h3>
                ...
                <div class="mt-8 pt-6 border-t dark:border-gray-700">
                    <button @click="save" class="bg-primary text-white px-6 py-2 rounded-md font-bold">{{ t('save_all_settings') }}</button>
                </div>
            </div>
        </div>
    `
...
app.component('profile-view', {
    ...
    template: `
        <div class="max-w-2xl mx-auto bg-white dark:bg-gray-800 p-8 rounded-xl shadow-sm">
            ...
            <form @submit.prevent="save" class="space-y-4">
                <div>
                    <label class="block text-sm font-medium dark:text-gray-300">{{ t('full_name') }}</label>
                ...
                <div>
                    <label class="block text-sm font-medium dark:text-gray-300">{{ t('telegram_id') }}</label>
                ...
                <div>
                    <label class="block text-sm font-medium dark:text-gray-300">{{ t('new_password_help') }}</label>
