...
const App = {
    setup() {
        ...
        return {
            user, token, lang, darkMode, view, t, toggleDarkMode, toggleLang,
        ...
        <div :class="{ 'dark': darkMode }" class="min-h-screen bg-gray-100 dark:bg-gray-900 transition-colors">
            ...
            <div v-else class="flex flex-col h-screen">
                ...
                <main class="flex-1 overflow-auto p-6">
                    <dashboard-view v-if="view === 'dashboard'" :stats="stats" :t="t"></dashboard-view>
...
app.component('login-form', {
    ...
    setup(props, { emit }) {
        ...
        return { form, submit: () => emit('login', form) };
    },
    template: `
        ...
        <div>
            ...
            <div v-if="stats" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border-l-4 border-red-500">
                    <div class="text-gray-500 dark:text-gray-400 text-sm font-medium">Новые</div>
                    <div class="text-3xl font-bold dark:text-white">{{ stats.by_status.new }}</div>
                </div>
            </div>
...
app.component('tasks-view', {
    ...
    setup(props, { emit }) {
        ...
        return { showForm, searchQuery, filterStatus, filteredTasks, statusColors, updateStatus, selectedTask, onDragEnd, print };
    },
    template: `
        <div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 mb-6 flex flex-wrap gap-4 no-print">
                ...
                <select v-model="filterStatus" class="border border-gray-300 dark:border-gray-600 rounded-md p-2 dark:bg-gray-700 dark:text-white">
                    ...
                    <option v-for="s in ['new', 'assigned', 'in_work', 'done', 'rejected']" :value="s">{{ t('status_' + s) }}</option>
                </select>
            </div>

            <!-- Kanban Board -->
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8 overflow-x-auto">
                <div v-for="status in ['new', 'assigned', 'in_work', 'done', 'rejected']" :key="status" class="bg-gray-100 dark:bg-gray-900 p-3 rounded-lg min-w-[250px] kanban-column">
                    ...
                    <draggable :list="tasks.filter(t => t.status === status)"
                        ...
                        <template #item="{element}">
                            <div @click="selectedTask = element"
                                 class="bg-white dark:bg-gray-800 p-4 rounded-md shadow-sm border-l-4 cursor-pointer hover:shadow-md transition-shadow"
                                 :class="statusColors[element.status]">
                                <div class="font-semibold text-gray-800 dark:text-white truncate">{{ element.title }}</div>
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm">
                ...
                <div class="space-y-8">
                    ...
                    <div>
                        ...
                        <div class="space-y-2">
                            <div v-for="(f, i) in localSettings.form_fields" :key="f.id" class="flex items-center space-x-2 bg-gray-50 dark:bg-gray-900 p-2 rounded">
                                <i class="fas fa-grip-lines text-gray-400"></i>
                                <input v-model="f.label" class="flex-1 border-none bg-transparent dark:text-white" placeholder="Название поля">
                                <select v-model="f.type" class="text-xs border rounded p-1 dark:bg-gray-700 dark:text-white">
                                ...
                                <label class="flex items-center text-xs dark:text-gray-400">
                                    <input type="checkbox" v-model="f.required" class="mr-1"> Обяз.
                                </label>
                                <button @click="localSettings.form_fields.splice(i,1)" class="text-red-500"><i class="fas fa-trash"></i></button>
                            </div>
