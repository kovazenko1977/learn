...
<body>
    <div id="app" class="h-full w-full relative" @mousemove="onMouseMove" @mouseup="onMouseUp">
        ...
        <!-- Desktop Area -->
        <div v-if="isAuthenticated && !booting" class="h-full w-full p-6 flex flex-col flex-wrap gap-4 items-start">
            ...
            <!-- Dynamic Windows -->
            <div v-for="win in windows" :key="win.id" v-show="!win.minimized" class="window"
                 :style="{ top: win.y + 'px', left: win.x + 'px', width: win.width + 'px', height: win.height + 'px', zIndex: win.zIndex }"
                ...
                <div class="window-header" @mousedown="onMouseDown($event, win.id)">
                    ...
                    <div class="flex gap-2">
                        <button @click="win.minimized = true" class="w-3 h-3 rounded-full bg-yellow-500 hover:bg-yellow-600"></button>
                ...
                <div class="flex-1 overflow-auto p-4 text-white">
                    ...
                    <div v-if="win.id === 'new_request'" class="space-y-4">
                        <div>
                            ...
                            <input v-model="newRequest.title" class="w-full bg-slate-900/50 border border-slate-700 rounded p-2 text-sm">
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs text-slate-400 mb-1 uppercase">Категория</label>
                                <select v-model="newRequest.category" class="w-full bg-slate-900/50 border border-slate-700 rounded p-2 text-sm">
                            ...
                            <div>
                                ...
                                <select v-model="newRequest.priority" class="w-full bg-slate-900/50 border border-slate-700 rounded p-2 text-sm">
                        ...
                        <div v-for="field in settings.form_fields" :key="field.id">
                            ...
                            <textarea v-if="field.type === 'textarea'" v-model="newRequest[field.id]" class="w-full bg-slate-900/50 border border-slate-700 rounded p-2 text-sm"></textarea>
                            <input v-else v-model="newRequest[field.id]" :type="field.type" class="w-full bg-slate-900/50 border border-slate-700 rounded p-2 text-sm">
                        </div>
                        <button @click="submitRequest" class="w-full bg-blue-600 py-2 rounded font-bold text-sm">Отправить заявку</button>
                    </div>
                    ...
                    <div v-if="win.id === 'my_requests'">
                        ...
                        <div v-else class="space-y-3">
                            <div v-for="req in requests" :key="req.id" class="bg-slate-800/50 border border-slate-700 rounded p-3 hover:bg-slate-800 transition cursor-pointer">
                                <div class="flex justify-between items-start mb-1">
                                    <span class="font-medium text-sm">{{ req.title }}</span>
                                    <span class="text-[10px] px-2 py-0.5 rounded" :class="statusClass(req.status)">{{ req.status }}</span>
                                </div>
                                <div class="text-[10px] text-slate-400">{{ req.created_at }} | {{ req.category }}</div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <!-- Taskbar -->
        <div v-if="isAuthenticated && !booting" class="taskbar">
             <button class="h-10 w-10 bg-blue-600 rounded flex items-center justify-center hover:bg-blue-700 transition">
             ...
             <div class="flex-1 px-4 flex gap-2 overflow-x-auto">
                 <button v-for="win in windows" :key="'tb-'+win.id"
                         ...
                         class="h-10 px-4 rounded bg-slate-800 text-white text-xs border border-slate-700 truncate max-w-[150px]"
