...
<body class="min-h-screen flex overflow-hidden">
    ...
    <aside class="w-72 border-r border-slate-200 bg-[#1e293b] flex flex-col p-8 z-50 transition-all duration-300 shadow-xl">
        ...
        <nav class="space-y-2 flex-grow">
            ...
            <?php endif; ?>
            <button onclick="showPage('quiz')" id="nav-quiz" class="nav-link w-full flex items-center space-x-4 px-6 py-4 rounded-2xl transition-all text-[11px] font-bold uppercase tracking-widest text-slate-400 hover:bg-white/5 hover:text-white">
                <i class="lucide-clipboard-check text-lg"></i> <span>Тестирование</span>
            </button>
    ...
    <script>
        ...
        function renderQuiz(content) {
            content.innerHTML = `
                <div class="mb-12 flex justify-between items-end">
                    <div>
                        <h1 class="text-3xl font-black text-white tracking-tight uppercase">Охрана Труда</h1>
                        <p class="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mt-2">Проверка знаний и тестирование</p>
                    </div>
                    <div class="flex space-x-4">
                        <button onclick="loadQuiz()" class="bg-indigo-600 text-white px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-indigo-500/20">Начать Тест</button>
                        <button onclick="loadQuizResults()" class="bg-slate-700 text-white px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest">Мои Результаты</button>
                    </div>
                </div>
                <div id="quiz-container" class="card-dark p-12 rounded-[2.5rem] bg-white">
                    <div class="text-center py-20">
                        <div class="text-6xl mb-8">📝</div>
                        <h2 class="text-2xl font-black text-slate-900 uppercase italic tracking-tighter">Система сертификации</h2>
                        <p class="text-sm font-medium text-slate-500 mt-4 max-w-md mx-auto">Для допуска к работе необходимо пройти ежегодную проверку знаний по вопросам охраны труда. Тест содержит 10 случайных вопросов.</p>
                        <button onclick="loadQuiz()" class="mt-10 bg-blue-600 text-white px-12 py-4 rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-blue-500/20 hover:scale-[1.02] transition-all">Запустить модуль</button>
                    </div>
                </div>
            `;
        }
        ...
        function loadQuiz() {
            ...
            container.innerHTML = '<div class="p-20 text-center"><div class="animate-spin inline-block w-8 h-8 border-4 border-indigo-500 border-t-transparent rounded-full"></div></div>';

            fetch('api.php?action=get_quiz_questions').then(r => r.json()).then(questions => {
            ...
            container.innerHTML = `
                <div class="mb-10">
                    <div class="flex justify-between items-end mb-4">
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Вопрос ${currentQuestionIndex + 1} из ${currentQuizQuestions.length}</p>
                        <p class="text-[10px] font-black text-blue-600 uppercase tracking-widest">${Math.round(progress)}%</p>
                    </div>
                    <div class="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div class="h-full bg-blue-600 transition-all duration-500" style="width: ${progress}%"></div>
                    </div>
                </div>
                <h3 class="text-xl font-bold text-slate-900 mb-10 leading-relaxed">${q.question}</h3>
                <div class="space-y-4 mb-12">
                    ${q.options.map((opt, idx) => `
                        <button onclick="selectQuizAnswer('${q.id}', ${idx})" class="w-full text-left p-6 rounded-2xl border-2 transition-all ${quizAnswers[q.id] === idx ? 'border-blue-600 bg-blue-50' : 'border-slate-100 hover:border-slate-300'}">
                            <div class="flex items-center space-x-4">
                                <div class="w-8 h-8 rounded-xl flex items-center justify-center font-black ${quizAnswers[q.id] === idx ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-400'}">${String.fromCharCode(65 + idx)}</div>
                                <p class="text-sm font-semibold ${quizAnswers[q.id] === idx ? 'text-blue-900' : 'text-slate-600'}">${opt}</p>
                            </div>
                        </button>
                    `).join('')}
                </div>
                <div class="flex justify-between">
                    <button onclick="prevQuizQuestion()" ${currentQuestionIndex === 0 ? 'disabled' : ''} class="px-8 py-4 rounded-xl border border-slate-200 text-[10px] font-black uppercase tracking-widest text-slate-400 disabled:opacity-30">Назад</button>
                    ${currentQuestionIndex === currentQuizQuestions.length - 1
                        ? `<button onclick="finishQuiz()" class="bg-emerald-600 text-white px-10 py-4 rounded-xl font-black uppercase tracking-widest text-[10px] shadow-lg shadow-emerald-500/20">Завершить тест</button>`
                        : `<button onclick="nextQuizQuestion()" class="bg-blue-600 text-white px-10 py-4 rounded-xl font-black uppercase tracking-widest text-[10px] shadow-lg shadow-blue-500/20">Следующий вопрос</button>`
                    }
                </div>
            `;
        }
        ...
        function finishQuiz() {
            if (Object.keys(quizAnswers).length < currentQuizQuestions.length) {
                if (!confirm('Вы ответили не на все вопросы. Завершить тест?')) return;
            }
        ...
        function renderQuizResult(result) {
            ...
            container.innerHTML = `
                <div class="text-center py-10">
                    <div class="text-6xl mb-8">${result.passed ? '🎉' : '❌'}</div>
                    <h2 class="text-3xl font-black text-slate-900 uppercase italic tracking-tighter">${result.passed ? 'Тест пройден' : 'Тест не пройден'}</h2>
                    <p class="text-sm font-medium text-slate-500 mt-4">Ваш результат: <span class="font-black text-slate-900">${result.score}%</span> (${result.correct_count} из ${result.total_questions})</p>
                    <div class="mt-12 flex justify-center space-x-4">
                        <button onclick="loadQuiz()" class="bg-blue-600 text-white px-10 py-4 rounded-xl font-black uppercase tracking-widest text-[10px]">Повторить тест</button>
                        <button onclick="loadQuizResults()" class="bg-slate-100 text-slate-600 px-10 py-4 rounded-xl font-black uppercase tracking-widest text-[10px]">История попыток</button>
                    </div>
                </div>
                <div class="mt-16 border-t border-slate-100 pt-12">
                    <h4 class="text-xs font-black text-slate-400 uppercase tracking-widest mb-8 text-center">Детальный разбор</h4>
                    <div class="space-y-6">
                        ${result.details.map((d, i) => `
                            <div class="p-6 rounded-2xl ${d.is_correct ? 'bg-emerald-50 border border-emerald-100' : 'bg-rose-50 border border-rose-100'}">
                                <div class="flex justify-between items-start mb-4">
                                    <p class="text-[10px] font-black ${d.is_correct ? 'text-emerald-600' : 'text-rose-600'} uppercase tracking-widest">Вопрос ${i+1}</p>
                                    <span class="text-[10px] font-black uppercase tracking-widest">${d.is_correct ? 'Верно' : 'Ошибка'}</span>
                                </div>
                                <p class="text-sm font-bold text-slate-900 mb-2">${d.question_text}</p>
                            </div>
                        `).join('')}
                    </div>
                </div>
        ...
        function loadQuizResults() {
            ...
            fetch('api.php?action=get_quiz_results').then(r => r.json()).then(results => {
                container.innerHTML = `
                    <div class="flex justify-between items-center mb-10">
                        <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest">История тестирования</h3>
                        <button onclick="loadQuiz()" class="text-[10px] font-black text-blue-600 uppercase tracking-widest">Пройти тест</button>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-[11px]">
                            <thead>
                                <tr class="text-slate-400 font-black uppercase tracking-widest border-b border-slate-100">
                                    <th class="pb-4">Сотрудник</th>
                                    <th class="pb-4">Дата</th>
                                    <th class="pb-4">Результат</th>
                                    <th class="pb-4">Балл</th>
                            ...
                            <tbody class="text-slate-600 font-bold">
                                ${results.reverse().map(r => `
                                    <tr class="border-b border-slate-50">
                                        <td class="py-4 text-slate-900">${r.user_name}</td>
                                        <td class="py-4 font-mono">${r.timestamp}</td>
                                        <td class="py-4">${r.correct_count} / ${r.total_questions}</td>
                                        <td class="py-4">${r.score}%</td>
                                        <td class="py-4">
                                            <span class="px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${r.passed ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}">
                                                ${r.passed ? 'Сдано' : 'Не сдано'}
                                            </span>
                                        </td>
                                    </tr>
                                `).join('') || '<tr><td colspan="5" class="py-10 text-center text-slate-400 uppercase tracking-widest font-black">Данных не обнаружено</td></tr>'}
                            </tbody>
                        </table>
                    </div>
                `;
