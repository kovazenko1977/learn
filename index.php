                ...
                <div class="bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] shadow-2xl w-full max-w-md">
                    ...
                    <div v-if="authView === 'login'">
                        ...
                        <div class="mt-4 text-center">
                            <button @click="authView = 'recover'" class="text-[10px] font-bold text-gray-400 hover:text-blue-600 uppercase tracking-widest">Забыли пароль?</button>
                        </div>
                    </div>

                    <div v-else>
                        <h3 class="text-xl font-black mb-6">Восстановление пароля</h3>
                        <div class="space-y-6">
                            <div>
                                <label class="block text-[10px] font-black uppercase text-gray-400 mb-2 tracking-widest">Email / Логин</label>
                                <input v-model="recoverUsername" type="text" class="w-full px-6 py-4 bg-gray-50 dark:bg-slate-800 border-2 border-transparent focus:border-blue-500 rounded-2xl outline-none transition-all font-bold">
                            </div>
                            <button @click="recoverPassword" class="w-full bg-blue-600 text-white py-4 rounded-2xl font-black shadow-xl shadow-blue-500/30 hover:bg-blue-700 active:scale-[0.98] transition-all">Сбросить пароль</button>
                            <button @click="authView = 'login'" class="w-full text-gray-500 font-bold text-xs uppercase tracking-widest">Назад к входу</button>
                        </div>
                    </div>

                    <div class="mt-10 text-center">
                        <button @click="seedData" class="text-[10px] font-bold text-gray-400 hover:text-blue-500 transition-colors uppercase tracking-widest">Инициализировать систему (Demo)</button>
                    </div>
                </div>
            </div>
                ...
                <div class="flex-1 flex flex-col overflow-hidden bg-gray-50 dark:bg-slate-950">
                    ...
                    <main class="flex-1 overflow-y-auto p-10">
                        ...
                        <div v-if="view === 'tasks'" class="max-w-7xl mx-auto">
                            <div class="flex justify-between items-center mb-10">
                                ...
                                <div class="flex items-center space-x-4">
                                    <div class="relative group">
                                        <button class="px-6 py-3 bg-white dark:bg-slate-900 border dark:border-slate-800 rounded-2xl text-xs font-black uppercase tracking-widest transition shadow-sm">{{ t('export') }}</button>
                                        <div class="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border dark:border-slate-700 overflow-hidden opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-30">
                                            <a :href="'api/export.php?token=' + token + '&format=csv'" class="block px-6 py-3 text-xs font-bold hover:bg-gray-50 dark:hover:bg-slate-700">Excel / CSV</a>
                                            <a :href="'api/export.php?token=' + token + '&format=print'" target="_blank" class="block px-6 py-3 text-xs font-bold hover:bg-gray-50 dark:hover:bg-slate-700">PDF / Print</a>
                                        </div>
                                    </div>
                            ...
                            <div v-else class="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-xl border dark:border-slate-800 overflow-hidden relative">
                                <div v-if="selectedTasks.length > 0" class="absolute top-4 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-8 py-3 rounded-2xl flex items-center space-x-6 shadow-2xl z-20 animate-in slide-in-from-top-8">
                                    <span class="text-xs font-black uppercase tracking-widest">{{ selectedTasks.length }} {{ t('selected') }}</span>
                                    <button @click="massAssign" class="text-xs font-black uppercase tracking-widest text-blue-400 hover:text-blue-300">{{ t('assign_selected') }}</button>
                                    <button @click="selectedTasks = []" class="text-xs font-black uppercase tracking-widest text-gray-400">×</button>
                                </div>
                                <table class="w-full text-left">
                                    <thead class="bg-gray-50/50 dark:bg-slate-800/30 border-b dark:border-slate-800">
                                        <tr>
                                            <th class="px-8 py-6">
                                                <input type="checkbox" @change="toggleSelectAll" :checked="selectedTasks.length === tasks.length" class="w-4 h-4 rounded border-gray-300">
                                            </th>
                                            <th class="px-6 py-6 text-[10px] font-black text-gray-400 uppercase tracking-widest">{{ t('id') }}</th>
                                    ...
                                    <tbody class="divide-y dark:divide-slate-800">
                                        <tr v-for="t in tasks" :key="t.id" class="hover:bg-blue-50/30 dark:hover:bg-blue-900/5 cursor-pointer transition">
                                            <td class="px-8 py-5" @click.stop>
                                                <input type="checkbox" v-model="selectedTasks" :value="t.id" class="w-4 h-4 rounded border-gray-300">
                                            </td>
                                            <td @click="openTask(t)" class="px-6 py-5 text-xs font-mono text-gray-400">#{{ t.id.substring(0, 6) }}</td>
                                            <td @click="openTask(t)" class="px-6 py-5">
                                            ...
                                            <td @click="openTask(t)" class="px-6 py-5"><span :class="priorityClass(t.priority)" class="px-3 py-1 rounded-lg text-[10px] font-black uppercase">{{ t.priority }}</span></td>
                                            <td @click="openTask(t)" class="px-6 py-5"><span :class="statusClass(t.status)" class="px-3 py-1 rounded-lg text-[10px] font-black uppercase">{{ t.status }}</span></td>
                                            <td @click="openTask(t)" class="px-6 py-5 font-bold text-sm text-gray-600 dark:text-gray-400">{{ t.executor_name || '—' }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
    ...
    <script>
        ...
        createApp({
            ...
            methods: {
                ...
                async login() {
                    const res = await fetch('api/login.php?action=login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(this.loginForm) });
                    if (res.ok) {
                ...
                async recoverPassword() {
                    const res = await fetch('api/login.php?action=recover', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username: this.recoverUsername }) });
                    if (res.ok) { alert('Instructions sent!'); this.authView = 'login'; } else alert('Error');
                },
                ...
                async seedData() { if (confirm('Инициализировать базу данных?')) { await fetch('api/seed.php?secret=seed123'); alert('Готово! Используйте admin / admin123'); } },
                toggleSelectAll(e) { if (e.target.checked) this.selectedTasks = this.tasks.map(t => t.id); else this.selectedTasks = []; },
                async massAssign() { const execId = prompt('Enter Executor ID:'); if (!execId) return; await fetch('api/tasks.php?action=assign', { method: 'POST', headers: { 'Authorization': 'Bearer ' + this.token, 'Content-Type': 'application/json' }, body: JSON.stringify({ id: this.selectedTasks, executor_id: execId }) }); this.fetchTasks(); this.selectedTasks = []; },
                initDraggable() {
                ...
                priorityClass(p) { const map = { 'Low': 'bg-gray-100 text-gray-500', 'Medium': 'bg-blue-100 text-blue-600', 'High': 'bg-orange-100 text-orange-600', 'Urgent': 'bg-red-100 text-red-600' }; return map[p] || 'bg-gray-100'; },
