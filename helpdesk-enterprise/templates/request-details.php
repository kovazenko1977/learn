<div class="hd-request-detail">
    ...
    <div class="hd-meta">
        ...
        <p><strong><?php _e('Ответственный:', 'helpdesk-enterprise'); ?></strong> <?php echo get_userdata($request->responsible_id)->display_name; ?></p>
        <p><strong><?php _e('Исполнитель:', 'helpdesk-enterprise'); ?></strong> <?php echo $request->executor_id ? get_userdata($request->executor_id)->display_name : __('Не назначен', 'helpdesk-enterprise'); ?></p>
        ...
        <p><strong><?php _e('Создана:', 'helpdesk-enterprise'); ?></strong> <?php echo esc_html($request->created_at); ?></p>
        <p><strong><?php _e('Срок (SLA):', 'helpdesk-enterprise'); ?></strong> <?php echo esc_html($request->deadline); ?></p>

        <?php if ($request->completed_at): ?>
            <p><strong><?php _e('Выполнена:', 'helpdesk-enterprise'); ?></strong> <?php echo esc_html($request->completed_at); ?></p>
            <?php
                $created = strtotime($request->created_at);
                $completed = strtotime($request->completed_at);
                $diff = $completed - $created;
                $hours = floor($diff / 3600);
                $minutes = floor(($diff % 3600) / 60);
            ?>
            <p><strong><?php _e('Фактическое время:', 'helpdesk-enterprise'); ?></strong> <?php echo sprintf(__('%d ч. %d мин.', 'helpdesk-enterprise'), $hours, $minutes); ?></p>
        <?php endif; ?>

        <?php
            $now = current_time('timestamp');
            $deadline = strtotime($request->deadline);
            if ($request->status !== 'completed' && $now > $deadline):
                $delay = $now - $deadline;
                $d_hours = floor($delay / 3600);
                $d_mins = floor(($delay % 3600) / 60);
        ?>
            <p style="color: red;"><strong><?php _e('Просрочка:', 'helpdesk-enterprise'); ?></strong> <?php echo sprintf(__('%d ч. %d мин.', 'helpdesk-enterprise'), $d_hours, $d_mins); ?></p>
        <?php elseif ($request->completed_at && strtotime($request->completed_at) > $deadline):
                $delay = strtotime($request->completed_at) - $deadline;
                $d_hours = floor($delay / 3600);
                $d_mins = floor(($delay % 3600) / 60);
        ?>
            <p style="color: red;"><strong><?php _e('Была просрочена на:', 'helpdesk-enterprise'); ?></strong> <?php echo sprintf(__('%d ч. %d мин.', 'helpdesk-enterprise'), $d_hours, $d_mins); ?></p>
        <?php endif; ?>
    ...
    <div class="hd-add-photo-section" style="margin-top: 10px;">
        <form id="hd-add-photo-form" enctype="multipart/form-data">
            <input type="file" name="photo" accept="image/*" required>
            <input type="hidden" name="request_id" value="<?php echo $request->id; ?>">
            <button type="submit" class="button"><?php _e('Добавить фото', 'helpdesk-enterprise'); ?></button>
        </form>
    </div>
            ...
            <?php if (current_user_can('hd_manage_dept_requests') || current_user_can('hd_manage_all')): ?>
                ...
                <?php if (current_user_can('hd_delete_data')): ?>
                    <div style="margin-top: 20px; border-top: 1px dashed #ccc; padding-top: 10px;">
                        <button id="hd-delete-request-btn" data-id="<?php echo $request->id; ?>" class="button button-link" style="color: #a00;"><?php _e('Удалить заявку навсегда', 'helpdesk-enterprise'); ?></button>
                    </div>
                <?php endif; ?>
            <?php endif; ?>
        </div>
                ...
                <li>
                    <strong><?php
                        $types = array(
                            ...
                            'request_deleted' => __('Заявка удалена', 'helpdesk-enterprise'),
