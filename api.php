...
switch ($action) {
    ...
    case 'get_quiz_questions':
        echo json_encode($quizManager->getQuestions(10));
        break;
    case 'submit_quiz':
        $userId = $auth->getCurrentUser()['id'];
        $userName = $auth->getCurrentUser()['name'];
        echo json_encode($quizManager->submitQuiz($userId, $userName, $input['answers']));
        break;
    case 'get_quiz_results':
        $userId = ($auth->getCurrentUser()['role'] === 'admin') ? null : $auth->getCurrentUser()['id'];
        echo json_encode($quizManager->getResults($userId));
        break;
    default:
